...
from pyqtgraph import SignalBlock, exceptionHandling, getConfigOption
from pyqtgraph.console import template_generic as ui_template
...
class ConsoleWidget(QWidget):
    ...
    _threadException = Signal(object)

    def __init__(
        self, parent=None, namespace=None, historyFile=None, text=None, editor=None
    ):
        """
        ==============  =============================================================================
        **Arguments:**
        ...
        self.frames = (
            []
        )  # stack frames to access when an item in the stack list is selected
    ...
    def catchAllExceptions(self, catch=True):
        ...
        """
        with SignalBlock(
            self.ui.catchAllExceptionsBtn.toggled, self.catchAllExceptions
        ):
            self.ui.catchAllExceptionsBtn.setChecked(catch)

        if catch:
            with SignalBlock(
                self.ui.catchNextExceptionBtn.toggled, self.catchNextException
            ):
    ...
    def stackItemDblClicked(self, item):
        ...
        subprocess.Popen(
            self.editor.format(fileName=fileName, lineNum=lineNum), shell=True
        )
    ...
    def updateSysTrace(self):
        ...
        if (
            not self.ui.catchNextExceptionBtn.isChecked()
            and not self.ui.catchAllExceptionsBtn.isChecked()
        ):
        ...
        if self.ui.onlyUncaughtCheck.isChecked():
            ...
            if sys.gettrace() is not None and sys.gettrace() != self.systrace:
                ...
                raise Exception(
                    "sys.settrace is in use; cannot monitor for caught exceptions."
                )
            else:
    ...
    def setStack(self, frame=None, tb=None):
        ...
        *frame* may either be a Frame instance or None, in which case the current
        ...
        for index, line in enumerate(traceback.extract_stack(frame)):
            ...
            self.ui.exceptionStackList.addItem(
                'File "%s", line %s, in %s()\n  %s' % line
            )
    ...
    def checkException(self, excType, exc, tb):
        ...
        if excType is KeyError:
            if filename.endswith("python2.7/weakref.py") and function in (
                "__contains__",
                "get",
            ):
                return False
        ...
        if excType is AttributeError:
            if filename.endswith("python2.7/collections.py") and function == "__init__":
                return False
            if filename.endswith("numpy/core/fromnumeric.py") and function in (
                "all",
                "_wrapit",
                "transpose",
                "sum",
                ...
                return False
            if filename.endswith("numpy/core/arrayprint.py") and function in (
                "_array2string"
            ):
                return False
        ...
        if excType is TypeError:
            if (
                filename.endswith("numpy/lib/function_base.py")
